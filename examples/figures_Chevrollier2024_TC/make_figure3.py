...
def calculate_bba(model_run, sza):
    ...
    bba = np.sum(model_run * irradiance_data) / np.sum(irradiance_data)
    return bba
...
for i, spec in enumerate(spectra.columns):
    ...
    without_laps_spectrum = emulator.run(
        [X_features[i][0], X_features[i][1], 0, X_features[i][3], 0, 0]
    )

    without_laps_bba = calculate_bba(without_laps_spectrum, sza)

    without_alg_spectrum = emulator.run(
        [
            X_features[i][0],
            X_features[i][1],
            0,
    ...
    without_dust_spectrum = emulator.run(
        [
            X_features[i][0],
            X_features[i][1],
            X_features[i][2],
    ...
    without_bc_spectrum = emulator.run(
        [
            X_features[i][0],
            X_features[i][1],
            X_features[i][2],
...
results_inversion["bba_drop_laps"] = (
    np.array(bba_diffs_bc) + np.array(bba_diffs_dust) + np.array(bba_diffs_algae)
)
results_inversion["bba_drop_algae_pct_total"] = (
    results_inversion["bba_drop_algae"].values
    / results_inversion["bba_drop_laps"].values
)
results_inversion["bba_drop_bc_pct_total"] = (
    results_inversion["bba_drop_bc"].values / results_inversion["bba_drop_laps"].values
)
results_inversion["bba_drop_dust_pct_total"] = (
    results_inversion["bba_drop_dust"].values
    / results_inversion["bba_drop_laps"].values
)

results_inversion["rf_algae"] = [
    bba_drop * mean_daily_rad for bba_drop in results_inversion.bba_drop_algae.values
]
results_inversion["rf_dust"] = [
    bba_drop * mean_daily_rad for bba_drop in results_inversion.bba_drop_dust.values
]
results_inversion["rf_bc"] = [
    bba_drop * mean_daily_rad for bba_drop in results_inversion.bba_drop_bc.values
]
# %%
...
ax1.bar(
    range(len(bba_diffs_bc)),
    results_inversion.sort_values("bba_drop_algae_pct_total")[
        "bba_drop_algae_pct_total"
    ].values,
    label="Algal blooms",
    color="darkred",
    alpha=0.7,
...
ax1.bar(
    range(len(bba_diffs_bc)),
    results_inversion.sort_values("bba_drop_algae_pct_total")[
        "bba_drop_bc_pct_total"
    ].values,
    bottom=results_inversion.sort_values("bba_drop_algae_pct_total")[
        "bba_drop_algae_pct_total"
    ].values,
    label="Dark particles",
    color="black",
...
# plt.savefig(f"{path_to_snowlaps}/examples/figures_Chevrollier2024_TC/fig03_Chevrollier2024_TC.png", dpi=300,
