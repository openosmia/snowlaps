...
class TestSnowlapsEmulator:
    ...
    test_features_training = np.load(
        "./tests/test_data/training_features_reduced.npy"
    ).tolist()

    test_targets_training = np.load("./tests/test_data/training_targets_reduced.npy")

    test_spectra_Chevrollier2024_path = (
        "./tests/test_data/Chevrollier_et_al_2024_TC_spectra_reduced.csv"
    )

    test_metadata_Chevrollier2024_path = (
        "./tests/test_data/Chevrollier_et_al_2024_TC_metadata_reduced.csv"
    )
    ...
    def test_load_emulator(self) -> None:
        emulator, emulator_wavelengths = self.my_emulator.load_emulator(
            self.emulator_path
        )
